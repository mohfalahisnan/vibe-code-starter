...
> Automated PR review with <PROJECT> design system awareness and confidence-based filtering.
...
For <PROJECT> projects, it also checks **design system compliance** — making sure we're using tokens, not hardcoded colors.
...
2. **Gather guidelines** — Find CLAUDE.md and <PROJECT> design system files
...
## <PROJECT> Design System Checks

For <PROJECT> projects, we check:
...
1. **Hardcoded color instead of token** (<PROJECT> design system)
   Using `<COLOR_PRIMARY>` directly — should use `var(--Ink)` or `var(--fg-primary)`
