...
Run an automated test suite against the <PROJECT> MCP server. This validates environment, authentication, all 10 tools, the resource endpoint, asset proxy, and error sanitization.
...
2. **Get the MCP API key:**
   ...
   - If not, ask the user for their MCP API key (generated in <PROJECT> settings)
...
curl -s -o /dev/null -w "%{http_code}" {BASE_URL}/api/brand/assets/download/logos/brandmark-Paper.svg
...
curl -s -D - -o /dev/null {BASE_URL}/api/brand/assets/download/logos/brandmark-Paper.svg | grep -i "cache-control\|x-content-type-options"
```
...
```
═══════════════════════════════════════
  <PROJECT> MCP Server Test Results
═══════════════════════════════════════
