...
**<PROJECT> Integration**: Every plan must account for our design system, component patterns, and brand voice requirements at the architecture level.
...
**Design System**: <PROJECT> (Ink/Paper/Accent palette, semantic tokens)
**Created**: YYYY-MM-DD
```
...
- [ ] Brand color (Accent) reserved for CTAs/active states only
- [ ] Interactive elements use React Aria Components
- [ ] Warm neutrals (Ink/Paper) instead of pure black/white
...
it('renders with <PROJECT> design tokens', () => {
render(<FeatureName />);
...
FeatureName
✕ renders with <PROJECT> design tokens (5ms)
\`\`\`
```

### 4. Component Pattern Reference

For UI components, specify which <PROJECT> pattern applies:
...
2. **Design System**: Are all UI elements mapped to <PROJECT> tokens?
...
2. Implement with <PROJECT> tokens
...
- **[Dashboard Widget Plan](dashboard-widget-plan.md)** — Complete example plan for a StatsWidget component with all <PROJECT> patterns

### References

- **[Component Patterns](claude%20config/Skills/writing-plans/references/component-patterns.md)** — Ready-to-use <PROJECT> component patterns (cards, buttons, inputs, modals, badges, animations)

---

_Adapted from obra/superpowers with <PROJECT> brand integration_
