...
This skill guides creation of distinctive, production-grade frontend interfaces that feel genuinely designed. When working on <PROJECT> projects, it automatically applies our warm neutrals philosophy and brand tokens.
...
## 2. <PROJECT> Design System Integration

When building for <PROJECT>, use our design tokens:

### Colors (Warm Neutrals)

```css
/* Primary Brand */
--brand-primary: <COLOR_ACCENT>;  /* Accent — CTAs, links, badges ONLY */

/* Warm Neutrals (NOT black/white) */
--Ink: <COLOR_PRIMARY>;       /* Dark backgrounds, primary text */
--Paper: <COLOR_BACKGROUND>;        /* Light backgrounds, light text */
...
| ----------- | ----------------------------- | ------------------- | -------------------------------------- |
| **Display** | <FONT_DISPLAY> | `font-display`      | Headlines, titles, hero text           |
| **Body**    | <FONT_BODY>    | `font-sans`         | Body text, paragraphs, inputs, tabs    |
| **Small**   | <FONT_BODY>    | `font-sans text-sm` | Labels, captions, hints, metadata      |
| **Accent**  | <FONT_MONO>                        | `font-accent`       | Digital/tech feel (max 2 per viewport) |

**Note**: We use <FONT_DISPLAY> consistently across the entire interface—no separate code or monospace fonts.
...
```css
/* <PROJECT> layered shadow using Ink (25,25,25) */
box-shadow:
...
- <PROJECT> Paper on Ink: 18.5:1 (AAA compliant)
...
/* <FONT_DISPLAY> supports tabular figures natively */
```
...
- Never use brand colors (Accent) for borders
...
- Use warm neutrals (Ink/Paper) for backgrounds and text
...
- <PROJECT> design tokens: `tailwind.config.ts`, `app/theme.css`, `lib/motion.tsx`
