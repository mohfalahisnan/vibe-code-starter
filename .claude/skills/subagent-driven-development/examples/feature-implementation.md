...
Implement a **Notification Bell** component for the <PROJECT> dashboard header with:
...
We're building a notification bell for the <PROJECT> dashboard header.
...
3. Badge uses brand color (Accent) per <PROJECT> guidelines
...
- Unread count badge (Accent background)
...
Both use same <PROJECT> patterns, no file conflicts.
```
...
4. **<PROJECT> compliance is non-negotiable** - Every review checks design system
